...
class URDFKinematicAnalyzer:
    ...
    def find_relations(self, samples=100, tol=0.95):
        ...
        for cond_name, cond in self.joints.items():
            ...
            # condition joint must have children
            children = [j for j in self.child_map[cond["child"]]]
            if not children:
                continue

            # parent side candidates
            ...
            for target in children:
                ...
                if source is None:
                    continue
                if "Finger" in source or "Finger" in target:
                    continue

                # sweep condition joint
                ...
                for interval in aligned_intervals:
                    relations.append({
                        "target": target,
                        "source": source,
                        "inverse": False,  # could check sign(dot)
                        "condition": cond_name,
                        "lower_limit": np.rad2deg(float(interval[0])),
                        "upper_limit": np.rad2deg(float(interval[1]))
                    })
        return relations
...
if __name__ == "__main__":
    ...
    #urdf_file = "urdf/pepper_pruned.urdf"  # replace with your URDF path
